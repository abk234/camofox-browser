#!/usr/bin/env bash
# Sync this fork with jo-inc/camofox-browser (fetch-only upstream; push only to origin).
set -euo pipefail
...
UPSTREAM_URL="${UPSTREAM_URL:-git@github.com:jo-inc/camofox-browser.git}"
...
Commands:
  status              Show how this branch compares to upstream/master
  sync [--rebase]     Fetch upstream and merge (default) or rebase onto upstream/master
...
Options:
  ...
  --branch <name>     Upstream branch to sync from (default: master)
...
ensure_remotes() {
  ...
  # Never allow pushes to the original camofox-browser repo.
...
cmd_push() {
  ...
  info "done (nothing was pushed to camofox-browser upstream)"
}
