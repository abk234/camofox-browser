#!/usr/bin/env python3
"""TCP proxy: listen on LAN (0.0.0.0:9377) → forward to camofox on 127.0.0.1:9377.

Needed because Docker Desktop (vpnkit/gvisor) + VPN often accepts LAN
connections to published ports but returns empty HTTP responses. Also useful
when the Node server is bound to loopback only (CAMOFOX_BIND_HOST=127.0.0.1).
"""
...
DEFAULT_LISTEN = ("0.0.0.0", 9377)
DEFAULT_TARGET = ("127.0.0.1", 9377)
PID_FILE_DEFAULT = os.path.join(os.path.dirname(__file__), ".camofox-lan-proxy.pid")
...
def serve(listen: tuple[str, int], target: tuple[str, int]) -> None:
    ...
    srv.listen(128)
    print(
        f"camofox-lan-proxy listening on {listen[0]}:{listen[1]} → {target[0]}:{target[1]}",
        flush=True,
    )
